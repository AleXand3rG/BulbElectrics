...
//Constructors:
uint32_t Varistor::type_=VARISTOR;
Varistor::Varistor(uint16_t x,uint16_t y) {
    index_ = Container::getCounterValue();
    designator_ = "Default Varistor";
    x_ = x;
    y_ = y;
    lineWidth_ = 2;
    value_ = "?value?";
...
Varistor::Varistor(uint32_t i,string d,uint16_t x,uint16_t y,uint16_t lw,string v,Color FC,Color LC,Color TC, float cv) {
    index_ = i;
    designator_ = d;
    x_ = x;
    y_ = y;
...
//iostream
void Varistor::save(ofstream& output) {
    output<<type_<<endl;
    ...
    output<<clampingVoltage_<<endl;
}

void Varistor::read(ifstream& input) {
    string tmp;
    uint8_t tmp_r=0;
    uint8_t tmp_g=0;
    uint8_t tmp_b=0;
    ...
    getline(input, tmp, '\n');
    clampingVoltage_ = stof(tmp);
    index_=Container::getCounterValue();
}
void Varistor::printInfo(ostream& os) const {
    os << index_+1 << ". ";
    ...
    os << "Clamping Voltage: " << clampingVoltage_ << endl;
...
// getters
uint32_t Varistor::getType() {
    return type_;
}
float Varistor::getClampingVoltage(){
    return clampingVoltage_;
}

// setters
bool Varistor::setClampingVoltage(float _value){
    clampingVoltage_ = _value;
    if(clampingVoltage_>=_value-1 && clampingVoltage_<=_value+1) return true;
    else return false;
}
// misc methods
string Varistor::parseSVGdefinition() {
    string parsed = "";
    uint8_t textSize = 15;
    ...
    parsed += "' x2='";
    parsed += to_string(x_ + 40);
    parsed += "' y2='";
    parsed += to_string(y_ + 30);
    parsed += "' style=\"stroke:";
    parsed += lineColor_.returnStringValue();
    parsed += ";stroke-width:";
    parsed += to_string(lineWidth_);
