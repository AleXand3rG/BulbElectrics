...
//Constructors:
uint32_t SmartBulb::type_=SMARTBULB;
SmartBulb::SmartBulb(uint16_t x,uint16_t y) {
    index_ = Container::getCounterValue();
    designator_ = "Default SmartBulb";
    x_ = x;
    y_ = y;
    lineWidth_ = 2;
    radius_ = 20;
...
SmartBulb::SmartBulb(uint32_t i,string d,uint16_t x,uint16_t y,uint16_t lw,uint16_t r,uint16_t p,Color FC,Color LC,Color TC,bool s,uint16_t t) {
    index_ = i;
    designator_ = d;
    x_ = x;
    y_ = y;
...
//iostream
void SmartBulb::printInfo(ostream& os) const {
    string ONOFF="OFF";
    if (state_) ONOFF="ON";
    ...
    os << "State: " << ONOFF << endl;
    os << "Intensity: " << intensity_ << '%' << endl;
    os << endl;
}
void SmartBulb::save(ofstream& output) {
    uint16_t ONOFF=0;
    ...
    output<<ONOFF<<endl;
    output<<intensity_<<endl;
}
void SmartBulb::read(ifstream& input) {
    string tmp;
    uint8_t tmp_state=0;
    uint8_t tmp_r=0;
    uint8_t tmp_g=0;
    ...
    getline(input, tmp, '\n');
    intensity_=uint16_t(stoi(tmp));
    index_=Container::getCounterValue();
}

//getters
uint32_t SmartBulb::getType() {
    return type_;
}
uint16_t SmartBulb::getIntensity() {
    return intensity_;
}

//setters
bool SmartBulb::setIntensity(uint16_t _value) {
    intensity_ = _value;
    if (intensity_ == _value) return true;
    else return false;
}

//misc methods
string SmartBulb::parseSVGdefinition() {
    string parsed = "";
    uint8_t textSize = 15;
    if (radius_>500) textSize = 40;
    if (radius_<=500 && radius_>200) textSize = 30;
    if (radius_<=200 && radius_>100) textSize = 25;
    if (radius_<=100 && radius_>50) textSize = 20;
    ...
    parsed += "' fill-opacity='";
    parsed += to_string(double(intensity_)/100);
    parsed += "' stroke='";
...
/*
 SVG SmartBulb template:

 left line:  <line x1='100' y1='100' x2='150' y2='100' style='stroke:rgb(255,0,0);stroke-width:2' />
 bulb:       <circle cx='200' cy='100' r='50' fill-opcity='0.5' stroke='green' stroke-width='2'
              fill='white' />
