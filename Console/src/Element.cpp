...
string Element::getColorString(uint8_t type) {
    switch (type) {
        case 1:
            return fillColor_.returnStringValue();
            break;
        case 2:
            return lineColor_.returnStringValue();
            break;
...
Color Element::getColor(uint8_t type) {
    switch (type) {
        ...
        case 3:
            return textColor_;
            break;
        default: Color newColor; newColor.setColorByName("Black"); return newColor; break;
    }
}
...
bool Element::setX(uint16_t _value) {
    if (_value>=0 && _value<=1000) x_ = _value;
    if (x_ == _value) return true;
    else return false;
}
bool Element::setY(uint16_t _value) {
    if (_value>=0 && _value<=1000) y_ = _value;
